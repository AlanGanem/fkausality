...
__all__ = ['get_distribution_var_factor_jaccard', 'pointwise_variance', 'estimate_mean_and_variance_from_neighbors',
           'sample_from_neighbors_continuous', 'PointwiseGaussianMixture', 'get_distribution_var_factor_jaccard',
           'pointwise_variance', 'estimate_mean_and_variance_from_neighbors', 'sample_from_neighbors_continuous',
           'PointwiseGaussianMixture']
...
# Cell
def estimate_mean_and_variance_from_neighbors(
    neighborhood_values,
    jac_dists,
    alpha = 1,
    variance_mapper = 'log',
    ...
    '''
    ref https://stats.stackexchange.com/questions/447626/mean-and-variance-of-a-mixture-distribution
    '''
    ...
    if pw_var.ndim > neighborhood_values.ndim:
        neighborhood_values = neighborhood_values[:, np.newaxis]
    var = np.average(neighborhood_values**2 + pw_var, weights = weights, axis = 0)
    #var = np.average(pw_var, weights = weights, axis = 0)
...
# Cell
class PointwiseGaussianMixture(DensityMixin, BaseEstimator):

    def __init__(
        self,
        pointiwse_distribution = 'normal',
        alpha = 1,
        scale_variance_pointwise = True,
        variance_mapper = 'log',
        min_var_factor = 1e-2,
        ...
        self.pointiwse_distribution = pointiwse_distribution
        self.alpha = alpha
